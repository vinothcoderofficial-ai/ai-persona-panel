...
describe("the AI panel shows what the model was asked", () => {
  ...
  it("renders the eleven numbers the prompt asks for", async () => {
    const harness = await mount({});
    try {
...
/**
 * The one claim on this screen that was not true.
 *
 * The header read "Persona policies and shopping traces are generated by a
 * language model and cached under data/cache/". Half of that is the strongest
 * evidence in the repository - `data/cache/traces/` is 80 trips and 973 turns
 * carrying a `model` field, one call per turn. The other half is false and this
 * project's own documents say so in three places: METHODOLOGY §5 and §12.8, and
 * README, all of which state that the four files in `data/cache/policies/` were
 * written by hand in S2 and that "an LLM designed the personas" would be a false
 * claim. A screen whose entire purpose is to make the AI checkable was making
 * the one claim a judge could check and catch.
 *
 * So this pins the corrected split rather than the words around it: the traces
 * are credited to the model, the policies are credited to the hand that wrote
 * them, and no sentence puts the two on the same side of "generated by".
 */
describe("the panel does not claim the model wrote the committed policies", () => {
  it("credits the traces to the model and the policies to the hand that wrote them", async () => {
    const harness = await mount({});
    try {
      const provenance = text(harness, "ai-provenance");
      expect(provenance).toContain("traces");
      expect(provenance).toMatch(/written by hand|hand-written/i);
      // The exact conflation that was there, and any re-wording of it that puts
      // policies and traces on one side of a single "generated by a model".
      expect(provenance).not.toMatch(
        /polic\w*\s+and\s+\w*\s*traces\s+(are|were)\s+(generated|written|produced)/i,
      );
    } finally {
      harness.unmount();
    }
  ...
  it("describes the button as asking and diffing, never as adopting", async () => {
    const harness = await mount({});
    try {
      const provenance = text(harness, "ai-provenance");
      expect(provenance).toMatch(/without adopting|never adopt|not adopted/i);
    } finally {
      harness.unmount();
    }
  });

  it("does not head the committed policy as the model's own answer", async () => {
    // "What it answered", over four hand-written files, is the same false claim
    // in three words. The heading has to survive a viewer reading only it.
    const harness = await mount({});
    try {
      expect(harness.container.textContent ?? "").not.toContain("What it answered");
    } finally {
      harness.unmount();
    }
  });
