 ...
 *
 * The AI in this project is real, and it is narrower than the pitch used to
 * say: `sim/slow_agent.py` walked twenty shoppers per persona through the store
 * one turn at a time, each turn a model call with a stated reason, and the 80
 * trips it produced have been committed under `data/cache/traces/` since S13 —
 * and *nothing rendered any of it*, so from a running instance there was no
 * evidence a model had ever been involved and no way to make one answer. That
 * is the gap this screen closes, and it closes it three ways:
 *
 ...
 *
 * **What the prompt did not produce.** `sim/policy.py` can turn an archetype
 * into a policy at temperature 0, but it never has for the four files in
 * `data/cache/policies/` — those were written by hand in S2, as METHODOLOGY §5
 * and §12.8 both state. This screen used to head them "what it answered" and
 * describe both halves of the cache as model output, which made the one claim
 * on the page a reader could check and catch. The prompt and the policy still
 * sit side by side, because that is the comparison the button makes; what
 * changed is that the page now says which of the two the model wrote.
 *
  ...
  return (
    <div data-testid="ai-panel" style={style.root}>
      <header style={{ marginBottom: 16 }}>
        ...
        {/*
          The provenance split, stated before anything else on the page, because
          it is the one thing about this screen that is easy to overclaim. The
          traces are the model's; the policies are not, and saying otherwise was
          a claim METHODOLOGY §5 and §12.8 already contradicted in writing.
        */}
        <div data-testid="ai-provenance" style={{ ...style.note, marginTop: 4, maxWidth: 780 }}>
          The shopping traces under <code style={style.monoStyle}>data/cache/traces/</code>{" "}
          are real model output — one call per turn, with the reason it gave. The four
          policies under <code style={style.monoStyle}>data/cache/policies/</code> are{" "}
          <strong>not</strong>: they were written by hand in S2, and the generator that
          would author them is built but has never authored the ones in use. This page
          shows the prompt that generator sends, the hand-written numbers the simulator is
          running, and a button that asks the model for a policy live and diffs its answer
          against the committed one — without adopting it.
        </div>
      </header>
...
function PromptPanel({ load }: { load: Load<AiPolicyResponse> }) {
  return (
    <div style={style.panel}>
      <div style={style.panelHeading}>What the model is asked</div>
      ...
      <div style={{ ...style.note, marginTop: 8 }}>
        ...
        real brands and categories, and fetched from the server — this is the string the
        button below sends, not a copy of the template.
      </div>
    </div>
  );
}
...
function PolicyPanel({
  ...
  return (
    <div style={style.panel}>
      <div style={style.panelHeading}>The policy being simulated</div>
      ...
      {/*
        Said again next to the numbers themselves, not only in the header. A
        viewer who scrolls straight to the eleven values sees the prompt on the
        left and reads them as its output; that is exactly the misreading the
        header was making on the screen's own behalf.
      */}
      <div style={{ ...style.note, marginTop: 8 }}>
        Hand-written in S2, not this prompt&apos;s output — and pre-registered: every lock
        in <code style={style.monoStyle}>predictions/</code> is hashed against the
        simulation these numbers drive.
      </div>
